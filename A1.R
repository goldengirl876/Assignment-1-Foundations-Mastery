...
par(mar = rep(2, 4))
log_attendees_distribution <- boxplot(log_attendees, data = log_attendees, main = "Log of Attendees", horizontal=TRUE, axes=TRUE, outline=FALSE)
...
count_in_location <- function(loc) {
  ...
  return(sprintf("There were %i protests in %s.", location_summary, loc))
}
...
count_for_purpose <- function(purpose) {
  ...
  matches <- str_detect(unlist(protest_purposes), purpose)
