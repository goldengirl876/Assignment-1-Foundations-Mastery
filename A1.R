...
median_attendees <- median(protests$Attendees
                           , na.rm = TRUE)
...
#When the mean and median are different, it shows how skewed the data are.
...
attendess_distribution <- boxplot(data = protests$Attendees, x = "Attendees")
...
test <- locations$Location[1]

test2 <- str_detect(test, "WA")
test3 <- str_detect(unlist(locations["Location"]), "WA")
num_in_wa <- sum(test3 == TRUE)
unlist(locations["Location"])
...
prop_in_wa <- prop_in_wa(protests)

# Reflection: Does the number of protests in Washington surprise you? 
#Why or why not?
#Yes, I assume that there would be way more.
...
count_in_location <- function(location)
  location_num <- sum(str_detect(locations, location))

# Use your function above to describe the number of protests in "Minneapolis"
# `minneapolis_summary`
test4 <- str_detect(unlist(locations["Location"]), "MN")
minneapolis_summary <- sum(test4 == TRUE)
unlist(locations["Location"])
...
july_2019 <- by_month_table_2019[6]
